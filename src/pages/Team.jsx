...
const Team = () => {
  ...
  const onInvite = async (data) => {
    try {
      const newMember = await api.inviteMember(data.email, data.name);
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-slate-500 dark:text-slate-400">Invite team members to view dashboards</p>
      </div>
      ...
      <div className="card dark:bg-slate-800 dark:border-slate-700 overflow-hidden p-0">
        {loading ? <div className="p-6 space-y-4"><Skeleton className="h-10" /><Skeleton className="h-10" /></div> : (
